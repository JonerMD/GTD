...
var DEFER_RE = /⏳\s*(\d{4}-\d{2}-\d{2})/;
var DUE_RE = /📅\s*(\d{4}-\d{2}-\d{2})/;
var CREATED_RE = /➕\s*(\d{4}-\d{2}-\d{2})/;
...
function parseTaskLine(line, file, lineNumber) {
  ...
  const deferDate = (_a = body.match(DEFER_RE)) == null ? void 0 : _a[1];
  const dueDate = (_b = body.match(DUE_RE)) == null ? void 0 : _b[1];
  const createdDate = (_c = body.match(CREATED_RE)) == null ? void 0 : _c[1];
  const text = body.replace(/➕\s*\d{4}-\d{2}-\d{2}/g, "").replace(/⏳\s*\d{4}-\d{2}-\d{2}/g, "").replace(/📅\s*\d{4}-\d{2}-\d{2}/g, "").replace(new RegExp(TAG_RE.source, TAG_RE.flags), "").replace(/\s+/g, " ").trim();
  if (text.length === 0)
    return null;
  return {
    text,
...
function isBlockedByAncestor(task, allTasks) {
  let currentParentLine = task.parentLine;
  while (currentParentLine !== void 0) {
...
var GTD_FOLDER = "GTD";
var PROJECTS_FOLDER = "GTD/Projects";
var INBOX_PATH = "GTD/Inbox.md";
var SINGLE_TASKS_PATH = "GTD/SingleTasks.md";
var COMPLETED_LOG = "GTD/Completed.md";
var DROPPED_LOG = "GTD/Dropped.md";
var SOMEDAY_LOG = "GTD/Someday.md";
var ARCHIVE_PROJECTS_FOLDER = "GTD/Archive/Projects";
var ARCHIVE_DROPPED_FOLDER = "GTD/Archive/Dropped";
function todayISO() {
...
function isProjectFileEmpty(content) {
...
function getGtdStatus(app, file) {
  var _a;
  const cache = app.metadataCache.getFileCache(file);
  const status = (_a = cache == null ? void 0 : cache.frontmatter) == null ? void 0 : _a["gtd-status"];
  if (status === "active" || status === "awaiting" || status === "completed" || status === "dropped" || status === "someday") {
    return status;
  }
  return void 0;
}
function isActiveProject(app, file) {
  const status = getGtdStatus(app, file);
  if (status === "active")
    return true;
  if (status === void 0 && file.path.startsWith(PROJECTS_FOLDER + "/")) {
    return true;
  }
  return false;
}
async function ensureSingleTasksFile(app) {
  const existing = app.vault.getAbstractFileByPath(SINGLE_TASKS_PATH);
  if (existing instanceof import_obsidian.TFile)
    return existing;
  await ensureFolder(app, GTD_FOLDER);
  const initial = "# Single Tasks\n\nStandalone opgaver der ikke h\xF8rer til et projekt. Hver linje er en task med valgfri due (\u{1F4C5}), defer (\u23F3) og tags (#).\n\n";
  return await app.vault.create(SINGLE_TASKS_PATH, initial);
}
async function appendSingleTask(app, data) {
  ...
  if (data.dueDate)
    parts.push(`\u{1F4C5} ${data.dueDate}`);
  if (data.deferDate)
    parts.push(`\u23F3 ${data.deferDate}`);
  parts.push(`\u2795 ${todayISO()}`);
...
async function appendToLog(app, logPath, logTitle, line) {
...
function getFrontmatterString(app, file, key) {
  var _a;
  const cache = app.metadataCache.getFileCache(file);
...
var TaskEditModal = class extends import_obsidian2.Modal {
  ...
  onOpen() {
    ...
    const sourceLabel = this.task.file.path === INBOX_PATH ? "Inbox" : this.task.file.path === SINGLE_TASKS_PATH ? "Single Tasks" : this.task.file.basename;
    ...
    this.tagsEl = contentEl.createEl("input", {
      type: "text",
      attr: { placeholder: "fx opkald \xE6rinder" }
    });
    ...
    this.dueEl = this.renderDateField(
      contentEl,
      "Due (\u{1F4C5})",
      (_a = this.task.dueDate) != null ? _a : ""
    );
    this.deferEl = this.renderDateField(
      contentEl,
      "Defer (\u23F3)",
      (_b = this.task.deferDate) != null ? _b : ""
    );
...
"${this.task.text}"`
    ...
    const cancelBtn = rightBtns.createEl("button", { text: "Annull\xE9r" });
  ...
  renderDateField(parent, label, initialValue) {
  ...
  async save() {
    ...
    if (!text) {
      new import_obsidian2.Notice("Task-teksten m\xE5 ikke v\xE6re tom.");
      return;
    }
    const leadingWs = (_b = (_a = this.task.rawLine.match(/^\s*/)) == null ? void 0 : _a[0]) != null ? _b : "";
    ...
    if (newDone) {
      ...
      const completedDate = this.task.done && existingCompleted ? existingCompleted[1] : todayISO();
    ...
    try {
      ...
      if (this.task.lineNumber >= lines.length) {
        new import_obsidian2.Notice("Kunne ikke finde linjen \u2014 filen er m\xE5ske \xE6ndret.");
        return;
      }
      lines[this.task.lineNumber] = newLine;
      await this.app.vault.modify(this.task.file, lines.join("\n"));
      new import_obsidian2.Notice("\u2713 Task opdateret");
      this.close();
    } catch (err) {
      console.error("Joner GTD: fejl ved task-redigering", err);
      new import_obsidian2.Notice("Kunne ikke gemme. Tjek konsollen for detaljer.");
    }
  }
  async deleteTask() {
    try {
      ...
      await this.app.vault.modify(this.task.file, lines.join("\n"));
      new import_obsidian2.Notice("\u{1F5D1} Task slettet");
      this.close();
    } catch (err) {
...
function isTaskAwaitingTagged(task) {
  return task.tags.some((t) => {
    const lower = t.toLowerCase();
    return lower === "afventer" || lower === "awaiting" || lower === "afventer-svar";
  });
}

// src/dashboard-view.ts
var VIEW_TYPE_DASHBOARD = "joner-gtd-dashboard";
var DashboardView = class extends import_obsidian3.ItemView {
  ...
  /* ============================================================
     TOOLBAR
     ============================================================ */
  async renderToolbar(parent) {
    const tabs = [
      { id: "inbox", label: "Inbox", icon: "\u{1F4E5}" },
      { id: "naeste", label: "N\xE6ste", icon: "\u{1F4CC}" },
      { id: "singles", label: "Single Tasks", icon: "\u{1F3AF}" },
      { id: "forecast", label: "Forecast", icon: "\u{1F4C5}" },
      { id: "projects", label: "Projekter", icon: "\u{1F4C1}" },
      { id: "tags", label: "Tags", icon: "\u{1F3F7}" },
      { id: "defer", label: "Udskudt", icon: "\u23F3" },
      { id: "review", label: "Review", icon: "\u{1F50D}" },
      { id: "someday", label: "Someday", icon: "\u{1F4A4}" },
      { id: "archive", label: "Arkiv", icon: "\u{1F4E6}" }
    ];
  ...
  async computePerspectiveCounts() {
    const result = {};
    const inboxFile = this.app.vault.getAbstractFileByPath(INBOX_PATH);
    if (inboxFile instanceof import_obsidian3.TFile) {
    ...
    if (singlesFile instanceof import_obsidian3.TFile) {
    ...
    for (const file of projectsFiles) {
      if (this.isDueForReview(file))
        reviewDue++;
    }
    if (reviewDue > 0)
      result.review = reviewDue;
    const somedayFiles = this.findFilesByStatus(["someday"], false);
    if (somedayFiles.length > 0)
      result.someday = somedayFiles.length;
    const archiveCount = this.findFilesByStatus(
      ["completed", "dropped"],
      false
    ).length;
    if (archiveCount > 0)
      result.archive = archiveCount;
    return result;
  }
  /* ============================================================
     SIDEBAR
     ============================================================ */
  async renderSidebar(parent) {
  ...
  renderTreeNode(parent, node, depth) {
    ...
    if (node.kind === "folder") {
      ...
      header.createSpan({ text: "\u{1F4C2}", cls: "gtd-project-icon" });
      ...
      const status = node.file ? getGtdStatus(this.app, node.file) : void 0;
      const icon = status === "awaiting" ? "\u23F3" : status === "someday" ? "\u{1F4A4}" : status === "completed" ? "\u2705" : status === "dropped" ? "\u{1F6AB}" : "\u{1F4C4}";
      itemEl.createSpan({ text: icon, cls: "gtd-project-icon" });
      itemEl.createSpan({ text: node.name, cls: "gtd-project-name" });
      if (node.openTasks !== void 0 && node.openTasks > 0) {
        itemEl.createSpan({
  ...
  async renderTagList(parent) {
    const tasksByTag = await this.collectTagsAcrossActive();
    const sorted = Array.from(tasksByTag.entries()).sort(
      (a, b) => a[0].localeCompare(b[0], "da")
    );
    if (sorted.length === 0) {
      const empty = parent.createDiv({ cls: "gtd-empty" });
  ...
  async renderNaesteMain(parent) {
    ...
    header.createEl("h2", { text: "\u{1F4CC} N\xE6ste", cls: "gtd-main-title" });
    header.createSpan({
      text: `${nextActions.length} n\xE6ste \xB7 ${singleTasks.length} single \xB7 ${awaitingFiles.length} afventer`,
      cls: "gtd-main-stats"
    });
    const sec1 = parent.createDiv({ cls: "gtd-section" });
    sec1.createEl("h3", {
      text: `\u{1F4CC} N\xE6ste handlinger (${nextActions.length})`
    });
    if (nextActions.length === 0) {
      const empty = sec1.createDiv({ cls: "gtd-section-empty" });
      empty.setText("Intet at g\xF8re lige nu. \u{1F389}");
    } else {
      nextActions.sort(
        (a, b) => a.file.basename.localeCompare(b.file.basename, "da")
      );
    ...
    secSingle.createEl("h3", {
      text: `\u{1F3AF} Single Tasks (${singleTasks.length})`
    });
    if (singleTasks.length === 0) {
      ...
      singleTasks.sort((a, b) => {
        var _a, _b;
        const aDue = (_a = a.dueDate) != null ? _a : "9999-99-99";
        const bDue = (_b = b.dueDate) != null ? _b : "9999-99-99";
    ...
    if (awaitingFiles.length === 0) {
      ...
      empty.setText("Intet at vente p\xE5.");
    } else {
  ...
  renderNextActionRow(parent, file, task) {
    ...
    if (task.dueDate)
      meta.createSpan({ text: `\u{1F4C5} ${task.dueDate}` });
  }
  renderAwaitingRow(parent, file) {
  ...
  /* ============================================================
     REVIEW PERSPEKTIV
     ============================================================ */
  async renderReviewMain(parent) {
    const allActive = this.findFilesByStatus(["active", "awaiting"], true);
    const dueForReview = [];
    for (const file of allActive) {
      if (this.isDueForReview(file))
        dueForReview.push(file);
    }
    const header = parent.createDiv({ cls: "gtd-main-header" });
    header.createEl("h2", { text: "\u{1F50D} Review", cls: "gtd-main-title" });
    header.createSpan({
    ...
    dueForReview.sort((a, b) => {
      var _a, _b;
      const ra = (_a = getFrontmatterString(this.app, a, "gtd-review")) != null ? _a : "";
      const rb = (_b = getFrontmatterString(this.app, b, "gtd-review")) != null ? _b : "";
  ...
  renderReviewRow(parent, file) {
    ...
    if (next) {
      ...
      if (overdue > 0) {
        meta.createSpan({
          text: `\u{1F4C6} ${overdue} dage forsinket`,
          cls: "gtd-overdue"
        });
      } else {
        meta.createSpan({ text: `\u{1F4C6} ${next}` });
      }
    }
    if (last) {
      meta.createSpan({ text: `\xB7 senest reviewed ${last}` });
    }
    if (interval) {
      meta.createSpan({ text: `\xB7 hver ${interval}d` });
    }
  ...
  async markReviewed(file) {
    ...
    if (interval) {
      ...
      if (Number.isFinite(days) && days > 0) {
        const next = /* @__PURE__ */ new Date(today + "T00:00:00");
    ...
    new import_obsidian3.Notice(`\u2713 ${file.basename} markeret som reviewed`);
  }
  isDueForReview(file) {
    const today = todayISO();
    const next = getFrontmatterString(this.app, file, "gtd-review");
  ...
  daysBetween(fromISO, toISO) {
    const from = (/* @__PURE__ */ new Date(fromISO + "T00:00:00")).getTime();
    const to = (/* @__PURE__ */ new Date(toISO + "T00:00:00")).getTime();
    return Math.floor((to - from) / (1e3 * 60 * 60 * 24));
  }
  isTaskDeferred(task) {
    if (!task.deferDate)
      return false;
  ...
  /* ============================================================
     ...
     ============================================================ */
  async renderDeferMain(parent) {
    const today = todayISO();
    const taskSources = [];
    const inbox = this.app.vault.getAbstractFileByPath(INBOX_PATH);
    if (inbox instanceof import_obsidian3.TFile)
      taskSources.push(inbox);
    const singles = this.app.vault.getAbstractFileByPath(SINGLE_TASKS_PATH);
    if (singles instanceof import_obsidian3.TFile)
      taskSources.push(singles);
    ...
    deferredTasks.sort(
      (a, b) => {
        var _a, _b;
        return ((_a = a.deferDate) != null ? _a : "").localeCompare((_b = b.deferDate) != null ? _b : "");
      }
    );
    ...
    header.createSpan({
      text: `${deferredTasks.length} tasks \xB7 ${deferredProjects.length} projekter`,
      cls: "gtd-main-stats"
    });
    if (deferredTasks.length === 0 && deferredProjects.length === 0) {
      const empty = parent.createDiv({ cls: "gtd-empty" });
      empty.createDiv({ text: "\u{1F389}", cls: "gtd-empty-icon" });
      empty.createDiv({
    ...
    if (deferredTasks.length > 0) {
      const sec1 = parent.createDiv({ cls: "gtd-section" });
      sec1.createEl("h3", {
        text: `\u{1F4CB} Udskudte tasks (${deferredTasks.length})`
      });
      for (const task of deferredTasks) {
        ...
        const body = row.createDiv({ cls: "gtd-action-row-body" });
        const projName = task.file.path === INBOX_PATH ? "Inbox" : task.file.basename;
        const projEl = body.createDiv({
          text: projName,
          cls: "gtd-action-row-project"
        });
    ...
    if (deferredProjects.length > 0) {
      const sec2 = parent.createDiv({ cls: "gtd-section" });
      sec2.createEl("h3", {
        text: `\u{1F4C1} Udskudte projekter (${deferredProjects.length})`
      });
      for (const dp of deferredProjects) {
        const row = sec2.createDiv({ cls: "gtd-awaiting-row" });
        row.createSpan({ text: "\u{1F4C1}" });
        row.createSpan({
  ...
  /* ============================================================
     ...
     ============================================================ */
  async renderForecastMain(parent) {
    var _a;
    ...
    const header = parent.createDiv({ cls: "gtd-main-header" });
    header.createEl("h2", { text: "\u{1F4C5} Forecast", cls: "gtd-main-title" });
    header.createSpan({
      text: `n\xE6ste ${days} dage`,
      cls: "gtd-main-stats"
    });
    ...
    if (!hasSources) {
      const empty = parent.createDiv({ cls: "gtd-empty" });
      empty.createDiv({ text: "\u{1F4C5}", cls: "gtd-empty-icon" });
      empty.createDiv({
        text: "Ingen kalender-kilder tilf\xF8jet endnu.",
        cls: "gtd-empty-text"
      });
      const settingsBtn = empty.createEl("button", {
        text: "\xC5bn indstillinger",
        cls: "gtd-action-btn mod-cta"
      });
      settingsBtn.style.marginTop = "12px";
      settingsBtn.onclick = () => this.callbacks.openSettings();
    }
    const { pastDue, byDate: gtdMap } = await this.collectGtdItemsForForecast(days);
    if (pastDue.length > 0) {
      const stats = header.querySelector(".gtd-main-stats");
      if (stats) {
        stats.textContent = `n\xE6ste ${days} dage \xB7 \u{1F534} ${pastDue.length} forsinket`;
      }
    }
    ...
    if (pastDue.length > 0) {
      ...
      dayEl.setText(`\u{1F534} FORSINKET
${pastDue.length} item${pastDue.length === 1 ? "" : "s"}`);
    ...
    for (let i = 0; i < days; i++) {
      ...
      const allItems = (_a = gtdMap.get(dateISO)) != null ? _a : [];
      const projectItems = allItems.filter((i2) => i2.source === "project");
      const singleItems = allItems.filter((i2) => i2.source === "single");
      if (events.length === 0 && projectItems.length === 0 && singleItems.length === 0)
        continue;
      renderedAny = true;
    ...
    if (!renderedAny && hasSources) {
      const empty = parent.createDiv({ cls: "gtd-empty" });
      empty.createDiv({ text: "\u{1F334}", cls: "gtd-empty-icon" });
      empty.createDiv({
        text: `Intet planlagt de n\xE6ste ${days} dage.`,
        cls: "gtd-empty-text"
      });
    }
  }
  renderForecastEvent(parent, ev, dateISO) {
    var _a, _b, _c, _d;
    ...
    if (!ev.allDay) {
      ...
      const endDate = (_b = (_a = ev.endISO) == null ? void 0 : _a.slice(0, 10)) != null ? _b : startDate;
      const endTime = (_d = (_c = ev.endISO) == null ? void 0 : _c.slice(11, 16)) != null ? _d : "";
      let timeText;
    ...
    if (ev.location) {
      el.createDiv({
        text: `\u{1F4CD} ${ev.location}`,
        cls: "gtd-forecast-event-loc"
      });
    }
  }
  renderForecastGtdItem(parent, item, showOverdue = false, todayISOStr = "") {
    ...
    if (showOverdue && todayISOStr) {
      ...
      if (daysOverdue > 0) {
        projectText += ` \xB7 ${daysOverdue} dag${daysOverdue === 1 ? "" : "e"} forsinket (${item.date})`;
      }
    }
    body.createDiv({
      text: projectText,
  ...
  async collectGtdItemsForForecast(days) {
    const byDate = /* @__PURE__ */ new Map();
    const pastDue = [];
    const today = todayISO();
    const lastDate = /* @__PURE__ */ new Date();
    lastDate.setDate(lastDate.getDate() + days);
    const lastISO = toLocalDateISO(lastDate);
    const add = (date, item) => {
      const fullItem = { ...item, date };
      if (date < today) {
    ...
    for (const file of sources) {
      ...
      const projLabel = file.path === INBOX_PATH ? "Inbox" : isSingle ? "Single Task" : file.basename;
      for (const task of tasks) {
        if (task.done)
          continue;
        if (task.dueDate) {
  ...
  async renderInboxMain(parent) {
    const inboxFile = await this.ensureInboxFile();
    const tasks = await this.tasksInFile(inboxFile);
    const open = tasks.filter((t) => !t.done);
    const header = parent.createDiv({ cls: "gtd-main-header" });
    header.createEl("h2", { text: "\u{1F4E5} Inbox", cls: "gtd-main-title" });
    header.createSpan({
      text: `${open.length} \xE5bne \xB7 ${tasks.length} i alt`,
      cls: "gtd-main-stats"
    });
  ...
  /* ============================================================
     ...
     ============================================================ */
  async renderSinglesMain(parent) {
    const file = await ensureSingleTasksFile(this.app);
    const tasks = await this.tasksInFile(file);
    const open = tasks.filter((t) => !t.done);
    const header = parent.createDiv({ cls: "gtd-main-header" });
    header.createEl("h2", {
      text: "\u{1F3AF} Single Tasks",
      cls: "gtd-main-title"
    });
    header.createSpan({
      text: `${open.length} \xE5bne \xB7 ${tasks.length} i alt`,
    ...
    if (tasks.length === 0) {
      const empty = parent.createDiv({ cls: "gtd-empty" });
      empty.createDiv({ text: "\u{1F3AF}", cls: "gtd-empty-icon" });
      empty.createDiv({
        text: 'Ingen single tasks endnu. Brug feltet nedenfor, eller "+ Med datoer" for at s\xE6tte deadline/defer.',
        cls: "gtd-empty-text"
      });
    } else {
  ...
  async renderSomedayMain(parent) {
    ...
    const lines = content.split("\n");
    const groups = [];
    let current = { heading: null, headingLine: null, items: [] };
    groups.push(current);
    for (let i = 0; i < lines.length; i++) {
    ...
    if (totalOpen === 0 && groups.every((g) => g.heading === null)) {
      ...
      empty.createDiv({
        text: "Ingen someday-items. Tilf\xF8j nedenfor, lav en gruppe, eller flyt et inbox-item hertil.",
        cls: "gtd-empty-text"
      });
    }
    for (const g of groups) {
      ...
      gh.createSpan({
        text: g.heading === null ? "Uden gruppe" : `\u{1F4C2} ${g.heading}`,
...
Items i gruppen slettes ikke \u2014 de flyttes bare ud af gruppen.`
  ...
  renderSomedayItemRow(parent, task) {
  ...
  renderSomedayGroupAddInput(parent, file, groupHeading) {
    const wrapper = parent.createDiv({ cls: "gtd-add-task" });
    const input = wrapper.createEl("input", {
      type: "text",
      cls: "gtd-add-task-input",
      attr: {
        placeholder: groupHeading === null ? "+ Tilf\xF8j someday-item\u2026" : `+ Tilf\xF8j til "${groupHeading}"\u2026`
      }
    });
    input.addEventListener("keydown", (e) => {
      if (e.key === "Enter") {
  ...
  async addSomedayGroup(file, name) {
    const content = await this.app.vault.read(file);
    const sep = content.endsWith("\n") ? "" : "\n";
...
`);
  ...
  async appendSomedayItemToGroup(file, groupHeading, text) {
    const content = await this.app.vault.read(file);
    const lines = content.split("\n");
    const line = `- [ ] ${text} \u2795 ${todayISO()}`;
    ...
    const hi = lines.findIndex(
      (l) => {
        var _a;
        return ((_a = l.match(/^##\s+(.+?)\s*$/)) == null ? void 0 : _a[1]) === groupHeading;
      }
    );
    if (hi === -1) {
      const sep = content.endsWith("\n") ? "" : "\n";
    ...
    let insertAt = end;
    while (insertAt > hi + 1 && lines[insertAt - 1].trim() === "")
      insertAt--;
  ...
  async renderTagMain(parent) {
    ...
    if (!this.selectedTag) {
      header.createEl("h2", {
        text: "\u{1F3F7} V\xE6lg et tag",
        cls: "gtd-main-title"
      });
      const empty = parent.createDiv({ cls: "gtd-empty" });
      empty.createDiv({
        text: "V\xE6lg et tag i listen til venstre for at se tasks.",
        cls: "gtd-empty-text"
      });
      return;
    }
    const tasksByTag = await this.collectTagsAcrossActive();
    const tasks = (_a = tasksByTag.get(this.selectedTag)) != null ? _a : [];
    const open = tasks.filter((t) => !t.done);
    header.createEl("h2", {
    ...
    header.createSpan({
      text: `${open.length} \xE5bne`,
      cls: "gtd-main-stats"
    });
    if (open.length === 0) {
      const empty = parent.createDiv({ cls: "gtd-empty" });
      empty.createDiv({
        text: "Ingen \xE5bne tasks med dette tag.",
        cls: "gtd-empty-text"
      });
      return;
    }
  ...
  async renderProjectMain(parent) {
    const header = parent.createDiv({ cls: "gtd-main-header" });
    if (!this.selectedProjectPath) {
      const persp = this.currentPerspective === "projects" ? "Projekter" : this.currentPerspective === "someday" ? "Someday / Maybe" : "Arkiv";
      header.createEl("h2", {
        text: persp,
        cls: "gtd-main-title"
      });
      const empty = parent.createDiv({ cls: "gtd-empty" });
      empty.createDiv({ text: "\u{1F448}", cls: "gtd-empty-icon" });
      empty.createDiv({
        text: "V\xE6lg et projekt i listen til venstre.",
        cls: "gtd-empty-text"
      });
      return;
    }
    ...
    if (!(file instanceof import_obsidian3.TFile)) {
      header.createEl("h2", {
        text: "(projekt findes ikke l\xE6ngere)",
        cls: "gtd-main-title"
      });
      return;
    }
  ...
  renderAddTaskInput(parent, target, placeholder) {
    const wrapper = parent.createDiv({ cls: "gtd-add-task" });
    const input = wrapper.createEl("input", {
      type: "text",
      cls: "gtd-add-task-input",
  ...
  /* ============================================================
     INSPECTOR
     ============================================================ */
  async renderInspector(parent, file) {
    var _a;
    parent.createEl("h3", { text: "Inspector" });
    const status = (_a = getGtdStatus(this.app, file)) != null ? _a : "active";
    ...
    this.renderDateField(parent, file, "N\xE6ste review", "gtd-review");
  ...
  renderDateField(parent, file, label, key) {
  ...
  findFilesByStatus(statuses, includeProjectsFolderWithoutStatus) {
    const result = [];
    for (const file of this.app.vault.getMarkdownFiles()) {
      ...
      if (includeProjectsFolderWithoutStatus && status === void 0 && file.path.startsWith(PROJECTS_FOLDER + "/")) {
        result.push(file);
      }
    }
    return result;
  ...
  buildTree(files, includeEmptyFolders = false) {
    ...
    for (const file of files) {
      ...
      for (let i = 0; i < parts.length - 1; i++) {
        ...
        let folder = (_b = current.children) == null ? void 0 : _b.find(
        ...
        if (!folder) {
          ...
          (_c = current.children) == null ? void 0 : _c.push(folder);
        }
        current = folder;
      }
      const node = {
      ...
      (_d = current.children) == null ? void 0 : _d.push(node);
    }
    this.sortTree(root);
  ...
  isActionable(task, allTasks) {
    if (task.done)
      return false;
    if (this.isTaskDeferred(task))
      return false;
    if (isBlockedByAncestor(task, allTasks))
      return false;
    return true;
  }
  /** Indsaml alle tasks fra Inbox + Single Tasks + aktive projekter, grupperet på tag. */
  async collectTagsAcrossActive() {
    const sources = [];
    const inbox = this.app.vault.getAbstractFileByPath(INBOX_PATH);
    if (inbox instanceof import_obsidian3.TFile)
      sources.push(inbox);
  ...
  async toggleTaskDone(task) {
    ...
    if (!line) {
      new import_obsidian3.Notice("Kunne ikke finde linjen \u2014 filen er m\xE5ske \xE6ndret.");
      return;
    }
    const newLine = task.done ? line.replace(/-\s*\[[xX]\]/, "- [ ]") : line.replace(/-\s*\[\s\]/, "- [x]");
    if (newLine === line) {
      new import_obsidian3.Notice("Kunne ikke opdatere linjen.");
      return;
    }
  ...
  async appendTaskToFile(file, text) {
  ...
  async ensureInboxFile() {
    ...
    const initial = "# Inbox\n\nUsorterede tanker, ideer og to-do's. Behandl regelm\xE6ssigt: hvad er det? Er det handlingsorienteret? Hvis ja: hvilket projekt h\xF8rer det til?\n\n";
    return await this.app.vault.create(INBOX_PATH, initial);
  }
  /** Skift status på et projekt — håndterer log + arkivering. */
  async changeStatus(file, newStatus) {
    ...
    const stampField = newStatus === "completed" ? "gtd-completed" : newStatus === "dropped" ? "gtd-dropped" : newStatus === "someday" ? "gtd-someday-since" : newStatus === "awaiting" ? "gtd-awaiting-since" : "gtd-activated";
    ...
    if (empty && archiveFolder) {
      ...
      this.selectedProjectPath = newPath;
      new import_obsidian3.Notice(`\u2705 Status \xE6ndret til ${newStatus}, filen er arkiveret.`);
    } else {
      new import_obsidian3.Notice(`\u2705 Status \xE6ndret til ${newStatus}.`);
    }
    void this.render();
  }
};
...
var DK_DAYS_SHORT = ["S\xF8n", "Man", "Tir", "Ons", "Tor", "Fre", "L\xF8r"];
var DK_MONTHS = [
  "jan",
  "feb",
  "mar",
...
function formatDayHeader(date, dayOffset) {
...
var SingleTaskModal = class extends import_obsidian3.Modal {
  ...
  onOpen() {
    const { contentEl } = this;
    contentEl.empty();
    contentEl.createEl("h2", { text: "\u{1F3AF} Ny Single Task" });
    contentEl.createEl("label", {
      text: "Hvad skal g\xF8res?",
      cls: "setting-item-name"
    });
    this.textEl = contentEl.createEl("input", {
      type: "text",
      attr: { placeholder: "Fx K\xF8b gave til mor" }
    });
    this.styleInput(this.textEl);
    this.textEl.focus();
    contentEl.createEl("label", {
      text: "Deadline (\u{1F4C5})",
      cls: "setting-item-name"
    });
    ...
    contentEl.createEl("label", {
      text: "Tags (valgfri, fx '\xE6rinder hus')",
      cls: "setting-item-name"
    });
    this.tagsEl = contentEl.createEl("input", {
      type: "text",
      attr: { placeholder: "\xE6rinder hus" }
    });
    this.styleInput(this.tagsEl);
    const buttonRow = contentEl.createDiv();
  ...
  async submit() {
    const text = this.textEl.value.trim();
    if (!text) {
      new import_obsidian3.Notice("Skriv en task-tekst f\xF8rst.");
      return;
    }
    const tags = this.tagsEl.value.trim().split(/\s+/).map((t) => t.replace(/^#/, "")).filter((t) => t.length > 0);
    const data = {
      text,
      dueDate: this.dueEl.value || void 0,
      deferDate: this.deferEl.value || void 0,
...
// src/action-sidebar-view.ts
var import_obsidian4 = require("obsidian");
var VIEW_TYPE_ACTION_SIDEBAR = "joner-gtd-action-sidebar";
var COLLAPSED_STORAGE_KEY = "joner-gtd:sidebar-collapsed";
var ActionSidebarView = class extends import_obsidian4.ItemView {
  constructor(leaf) {
    super(leaf);
    this.debouncedRender = (0, import_obsidian4.debounce)(
      () => void this.render(),
      250,
      true
    );
  ...
  renderSectionHeader(parent, key, label, count) {
  ...
  async render() {
    ...
    const tasks = [];
    const inbox = this.app.vault.getAbstractFileByPath(INBOX_PATH);
    if (inbox instanceof import_obsidian4.TFile) {
      const t = await this.tasksInFile(inbox);
      for (const task of t) {
        if (this.isActionable(task, t))
          tasks.push(task);
      }
    }
    for (const file of activeFiles) {
    ...
    const singleTasks = [];
    const singlesFile = this.app.vault.getAbstractFileByPath(SINGLE_TASKS_PATH);
    if (singlesFile instanceof import_obsidian4.TFile) {
    ...
    const awaitingTaggedTasks = [];
    const checkForAwaitingTag = (taskList) => {
      for (const t of taskList) {
        if (t.done)
          continue;
        if (isTaskAwaitingTagged(t))
          awaitingTaggedTasks.push(t);
      }
    };
    if (inbox instanceof import_obsidian4.TFile) {
    ...
    const sec1 = this.renderSectionHeader(
      root,
      "next",
      "\u{1F4CC} N\xE6ste handlinger",
      tasks.length
    );
    const sec1Content = sec1.createDiv({ cls: "gtd-as-section-content" });
    if (tasks.length === 0) {
      sec1Content.createDiv({ text: "Intet at g\xF8re. \u{1F389}", cls: "gtd-as-empty" });
    } else {
      for (const task of tasks) {
    ...
    awaitingFiles.sort(
      (a, b) => a.basename.localeCompare(b.basename, "da")
    );
  ...
  renderTaskRow(parent, task) {
    ...
    for (const tag of task.tags)
      metaParts.push(`#${tag}`);
    if (task.dueDate)
      metaParts.push(`\u{1F4C5} ${task.dueDate}`);
    body.createDiv({ text: metaParts.join(" \xB7 "), cls: "gtd-as-task-meta" });
  }
  renderAwaitingRow(parent, file) {
  ...
  findFilesByStatus(statuses) {
    ...
    for (const file of this.app.vault.getMarkdownFiles()) {
      ...
      if (status === void 0 && statuses.includes("active") && file.path.startsWith(PROJECTS_FOLDER + "/")) {
        result.push(file);
      }
    }
    return result;
...
async function completeProject(app) {
  await transitionProject(app, {
    targetStatus: "completed",
    logPath: COMPLETED_LOG,
    logTitle: "Afsluttede projekter",
    formatLogLine: (name) => `- [x] [[${name}]] \u2014 afsluttet ${todayISO()}`,
    archiveFolder: ARCHIVE_PROJECTS_FOLDER,
    markTasksDone: true,
    verb: "afsluttet"
  });
}
async function dropProject(app) {
  await transitionProject(app, {
    targetStatus: "dropped",
    logPath: DROPPED_LOG,
    logTitle: "Droppede projekter",
    formatLogLine: (name) => `- ~~[[${name}]]~~ \u2014 droppet ${todayISO()}`,
    archiveFolder: ARCHIVE_DROPPED_FOLDER,
    markTasksDone: false,
    verb: "droppet"
  });
}
async function defererProjectToSomeday(app) {
  await transitionProject(app, {
    ...
    archiveFolder: null,
    // someday-projekter arkiveres ikke; rige sider bliver
    markTasksDone: false,
    verb: "parkeret som someday"
  });
}
async function transitionProject(app, opts) {
  const file = app.workspace.getActiveFile();
  if (!file) {
    new import_obsidian5.Notice("\xC5bn et projekt f\xF8rst.");
    return;
  }
  if (!isActiveProject(app, file)) {
    new import_obsidian5.Notice("Denne fil er ikke markeret som et aktivt projekt.");
    return;
  }
  const name = file.basename;
  ...
  const stampField = opts.targetStatus === "completed" ? "gtd-completed" : opts.targetStatus === "dropped" ? "gtd-dropped" : "gtd-someday-since";
  updated = setFrontmatterFields(updated, {
  ...
  if (empty && opts.archiveFolder) {
    ...
    new import_obsidian5.Notice(`\u2705 Projekt ${opts.verb} og arkiveret.`);
    return;
  }
  if (empty && !opts.archiveFolder) {
    new import_obsidian5.Notice(`\u{1F4A4} Projekt ${opts.verb}.`);
    return;
  }
  new import_obsidian5.Notice(`\u2705 Projekt ${opts.verb} (siden bliver \u2014 har andet indhold).`);
}
var NewProjectModal = class extends import_obsidian5.Modal {
  ...
  onOpen() {
    const { contentEl } = this;
    contentEl.empty();
    contentEl.createEl("h2", { text: "\u{1F4C1} Nyt projekt" });
    contentEl.createEl("label", {
      text: "Projektnavn",
      cls: "setting-item-name"
    });
    this.nameEl = contentEl.createEl("input", {
      type: "text",
      attr: { placeholder: "Fx Renovering af badev\xE6relse" }
    });
    ...
    this.folderEl = contentEl.createEl("input", {
      type: "text",
      attr: {
        placeholder: "Fx Hospital \xB7 Hjemme \xB7 Firma (tom = ingen)",
  ...
  async submit() {
    ...
    if (!name) {
      new import_obsidian5.Notice("Skriv et projektnavn f\xF8rst.");
      return;
    }
    const safe = safeFileName(name);
    if (!safe) {
      new import_obsidian5.Notice("Ugyldigt projektnavn.");
      return;
    }
    const subfolder = this.folderEl.value.trim().replace(/^\/+|\/+$/g, "");
    const targetFolder = subfolder ? `${PROJECTS_FOLDER}/${subfolder}` : PROJECTS_FOLDER;
    const path = `${targetFolder}/${safe}.md`;
    const existing = this.app.vault.getAbstractFileByPath(path);
    if (existing) {
      new import_obsidian5.Notice(`Et projekt "${safe}" findes allerede her.`);
      return;
    }
    await ensureFolder(this.app, targetFolder);
    const file = await this.app.vault.create(path, projectTemplate(safe));
    new import_obsidian5.Notice(`\u{1F4C1} Oprettet: ${safe}`);
    this.submitted = true;
    this.onResult(file);
    this.close();
  }
...
var JonerGTDSettingTab = class extends import_obsidian6.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian6.Setting(containerEl).setName("Antal dage frem").setDesc("Hvor mange dage Forecast skal d\xE6kke (default 30).").addText(
      (text) => text.setPlaceholder("30").setValue(String(this.plugin.settings.forecastDays)).onChange(async (value) => {
    ...
    new import_obsidian6.Setting(containerEl).setName("Refresh-interval (minutter)").setDesc("Hvor ofte kalender-data hentes p\xE5 ny.").addText(
    ...
    containerEl.createEl("p", {
      text: "Tilf\xF8j \xE9n eller flere offentliggjorte iCal-URL'er fra Apple Calendar (h\xF8jreklik p\xE5 kalender \u2192 Share Calendar \u2192 Public Calendar). URL'er der starter med webcal:// konverteres automatisk til https://.",
      cls: "setting-item-description"
    });
    ...
    new import_obsidian6.Setting(containerEl).addButton(
      (btn) => btn.setButtonText("+ Tilf\xF8j kalender").setCta().onClick(async () => {
    ...
    new import_obsidian6.Setting(containerEl).addButton(
      (btn) => btn.setButtonText("\u21BB Refresh alle kalendere nu").onClick(async () => {
        await this.plugin.calendarService.refreshAll(true);
      })
    ).addExtraButton(
      (btn) => btn.setIcon("info").setTooltip("Henter alle aktive kalendere fra deres URL'er")
    );
  }
  renderSourceRow(containerEl, source) {
...
var CalendarService = class {
  ...
  async refreshAll(force = false) {
    ...
    const intervalMs = this.plugin.settings.refreshIntervalMinutes * 60 * 1e3;
    const now = Date.now();
    const promises = sources.map(async (source) => {
      var _a;
      const last = (_a = this.lastFetched.get(source.id)) != null ? _a : 0;
  ...
  expandRecurring(event, rangeStart, rangeEnd, source, out) {
    var _a;
  ...
  makeEvent(event, startJs, endJs, source) {
    var _a, _b, _c;
    const allDay = event.startDate.isDate;
    return {
      uid: (_a = event.uid) != null ? _a : "",
      summary: (_b = event.summary) != null ? _b : "(uden titel)",
      location: (_c = event.location) != null ? _c : void 0,
  ...
  eventOverlapsDate(ev, dateISO) {
    var _a, _b, _c;
    if (ev.allDay) {
      const start = ev.startISO;
      const endExclusive = (_a = ev.endISO) != null ? _a : start;
      return dateISO >= start && dateISO < endExclusive;
    }
    const startDate = ev.startISO.slice(0, 10);
    const endDate = (_c = (_b = ev.endISO) == null ? void 0 : _b.slice(0, 10)) != null ? _c : startDate;
...
function toLocalDateTime(d) {
  return `${toLocalDate(d)}T${pad3(d.getHours())}:${pad3(d.getMinutes())}`;
}

// src/main.ts
var CALENDAR_UPDATED_EVENT = "joner-gtd:calendar-updated";
var JonerGTDPlugin = class extends import_obsidian8.Plugin {
  async onload() {
    console.log("Joner GTD: indl\xE6st");
    ...
    this.registerView(
      ...
      (leaf) => new DashboardView(leaf, {
        ...
        refreshCalendars: (force = false) => this.calendarService.refreshAll(force),
    ...
    this.addCommand({
      ...
      callback: async () => {
        const file = await createProject(this.app);
        if (file) {
          const dashboardOpen = this.app.workspace.getLeavesOfType(VIEW_TYPE_DASHBOARD).length > 0;
          if (!dashboardOpen) {
    ...
    this.addCommand({
      ...
      name: "GTD: Park\xE9r projekt som Someday/Maybe",
  ...
  async onunload() {
    var _a;
    console.log("Joner GTD: afl\xE6st");
    (_a = this.calendarService) == null ? void 0 : _a.stopAutoRefresh();
  }
  async loadSettings() {
  ...
  async appendToInbox(text) {
    ...
    const next = current + (needsNewline ? "\n" : "") + taskLine + "\n";
    await this.app.vault.modify(file, next);
    new import_obsidian8.Notice(`\u{1F4E5} Tilf\xF8jet til Inbox: "${text}"`);
  }
  async ensureInboxFile() {
    const existing = this.app.vault.getAbstractFileByPath(INBOX_PATH);
    if (existing instanceof import_obsidian8.TFile)
      return existing;
    await ensureFolder(this.app, GTD_FOLDER);
    const initial = "# Inbox\n\nUsorterede tanker, ideer og to-do's. Behandl regelm\xE6ssigt: hvad er det? Er det handlingsorienteret?\n\n";
    return await this.app.vault.create(INBOX_PATH, initial);
  }
};
var QuickCaptureModal = class extends import_obsidian8.Modal {
  constructor(app, onSubmit) {
    super(app);
    this.onSubmit = onSubmit;
  }
  onOpen() {
    const { contentEl } = this;
    contentEl.empty();
    contentEl.createEl("h2", { text: "\u{1F4E5} Hurtig capture til Inbox" });
    this.inputEl = contentEl.createEl("input", {
      type: "text",
      attr: {
        placeholder: "Hvad skal du huske? (#tag virker ogs\xE5)"
      }
    });
    this.inputEl.style.width = "100%";
  ...
  async submit() {
    ...
    if (!text) {
      new import_obsidian8.Notice("Skriv noget f\xF8r du gemmer");
      return;
    }
    this.close();
